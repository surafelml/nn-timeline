"""
Script defines the top level dataclass to configure model and task params
"""
...
class DataclassBase:  # FairseqDataclass
    ...
    def _get_default(self, attribute_name: str) -> Any:
        ...
        # if not isinstance(f.default_factory, _MISSING_TYPE):  # todo: fix missing type
...
@dataclass
class CommonConfig(DataclassBase):
    ...
    # used for a particular purpose or task, such as those dedicated for
    ...
    # Note: mixed precision, dist training, etc
...
# Note: move to separate dataclass_dataparallel.py ? for clarity
#Removed Distributed Training params for less confusion - NO
@dataclass
class DistributedTrainingConfig(DataclassBase):
    """
    Class for ???
    """
...
@dataclass
class DatasetConfig(DataclassBase):
...
@dataclass
class OptimizationConfig(DataclassBase):
    max_epoch: int = field(
...
@dataclass
class CheckpointConfig(DataclassBase):
    save_dir: str = field(
...
@dataclass
class GenerationConfig(DataclassBase):
    beam: int = field(
    ...
    # Note: schedule when to add the feature - NOW as this feature is needed
    sampling: bool = field(
        default=False,
...
# Note: merge with Generation
@dataclass
class InteractiveConfig(DataclassBase):
...
@dataclass
class CommonEvalConfig(DataclassBase):
...
@dataclass
class DataclassConfig(DataclassBase):
    common: CommonConfig = CommonConfig()
    common_eval: CommonEvalConfig = CommonEvalConfig()
    #decided to keep it with Notes as its crucial for faster training
    distributed_training: DistributedTrainingConfig = DistributedTrainingConfig() # focus on using pytorch based DDP
