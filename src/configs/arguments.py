"""
Replicates and simplify
                        fairseq/options.py
using dataclass for common/shared and transformer tasks.
"""
...
from src.configs.dataclass import (
    CheckpointConfig,
    CommonConfig,
    CommonEvalConfig,
    DatasetConfig,
...
from .dataclass_parser import gen_parser_from_dataclass # convert a dataclass instance to tailing parser arguments.
...
def get_parser(desc, default_task="translation"):
    # Note: removing from v1
    # Before creating the true parser,
    # we need to import optional user module  in order to eagerly import custom tasks, optimizers, architectures, etc.
    ...
    gen_parser_from_dataclass(parser, CommonConfig())  # convert dataclass/CommonConfig instance to tailing parser args.

    #Note: removing this in v1.0, as the parsing the optional module above
    ...
    # Note: removing from v1 to simplify.
    ...
    #     choices=TASK_REGISTRY.keys(),  # populated
    #     help="task",
    # )
    # fmt: on

    parser.add_argument("--task", default=default_task, help="task name.")
    return parser


# FIXME: for consistency this should go to configs/configs/dataclass, only keep relevant for v1
...
#
# # TODO: HERE - figuring out - what does it do - why to remove it - SDE not ML.
# # Note: not in v1.0
...
def add_model_args(parser):
    ...
    from src.models import ARCH_MODEL_REGISTRY
...
# Note: not used except in non translation tasks
# def get_args(
#     data: Union[str, Path],
