"""Base class for attributes (fetching, ?)
TODO: include all model configurations fo QaD.
    i. copy relevant configs,
    ii. after experiments - cleanup and simplify
    NG: we could use standard argument declaration, however, we keep the @dataclass approach for future best practice.
"""
...
#
# #TODO: simplify and remove this class
# @dataclass
...
# from configs/transformer_config.py
@dataclass
class EncDecBaseConfig(DataclassBase):
...
# HERE: clean this
@dataclass
class TransformerConfig(DataclassBase):     # TnnConfig ?
    activation_fn: constants.ChoiceEnum(constants.get_available_activation_fns()) = field(
        default="relu",
    ...
    # note: remove adaptive related features
    ...
    # note: no need - used for backward compatibility
    ...
    def from_namespace(cls, args):
        ...
        if not isinstance(args, cls):
            seen = set()
            config = cls()
            # currently, we can go generically from DC (DictConfigs) fields to args hierarchically
            ...
            for fld in fields(cls):
                ...
                if fld.name == "decoder":
                    if hasattr(args, "decoder"):
                        #  in some cases, the args we receive is already structured (as DictConfigs),
            ...
            args_dict = args._asdict() if hasattr(args, '_asdict') else vars(args) \
                if hasattr(args, '__dict__') else {}  # namedtupled doesn't have __dict__ :-/
