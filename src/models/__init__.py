# TODO: find proper place. See OpenNMT-tf approach for config: sitMT/models.py
# What is the design principle behind this module, how is fairseq structured?
# - models, modules, task, configurations, data processing
# learning frame others frameworks, and progressively including other tasks, what can we simplify ?
# SEE https://github.com/OpenNMT/OpenNMT-tf/blob/master/opennmt/models/__init__.py
...
from src.data.data_utils import merge_with_parent
...
from .model_base_encoder import ModelBaseEncoder
from .model_base_encoder import ModelBaseEncoder
from .model_base_decoder import ModelBaseDecoder
...
def build_model(cfg: DataclassBase, task):
...
def register_model(name, dataclass=None):
    ...
    def register_model_cls(cls):
        ...
        if not issubclass(cls, ModelBase): # BaseFairseqModel):
            raise ValueError(
                "Model ({}: {}) must extend BaseFairseqModel".format(name, cls.__name__)
            )
        MODEL_REGISTRY[name] = cls
        if dataclass is not None and not issubclass(dataclass, DataclassBase):
            raise ValueError(
