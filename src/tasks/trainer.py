"""
SCRIPT: Train a network across multiple GPUs. Trainer instance accessed by the train cli/api.

STATUS:
- Errors are cleaned.
- TODO: create a simple trainer - approach - remove unncessary parts before simplifying
...
from src.metrics import metrics, meters
from src.utils.file_io import PathManager
from src.tasks import checkpoint
from omegaconf import OmegaConf
...
class Trainer(object):
    ...
    More on DDP: https://www.run.ai/guides/multi-gpu/pytorch-multi-gpu-4-techniques-explained
    enables to perform data parallelism across GPU's and physical
    machines and can be combined with model parallelism.
    This method relies on the DistributedDataParallel class.

    Clearly DDP is more efficient than DataParallel, as it manages >1 processor and threads.

    *Need such a note for all features.

    TODO: i. keep and use DDP based data/model parallel,
          ii. despite its pros remove FSDP/FullyShardedDataParallel based data/model/parameter & optimizer state sharding, as it requires
          more work (see https://github.com/facebookresearch/fairseq/blob/main/examples/fully_sharded_data_parallel/README.md)
    """

    def __init__(self, cfg: DataclassConfig, task, model, criterion):  #, quantizer=None):

        # note: if using OmegaConf - we don't need the convert option
        ...
        # catalog shared parameters - for cross device check
        shared_params = _catalog_shared_params(model) # Note: task ?
        ...
        # fixme: HERE 
        # note: parallel training params ?
        # if self.is_fsdp:
        #     if self.cfg.common.bf16:  # used for tpu
        #         raise ValueError(
        ...
        # if not self.is_fsdp:  # note: remove condition as the top variant is removed
        #     if cfg.common.fp16:
    ...
    def _build_optimizer(self):
        ...
        # note: removing fp16 approach and associated code (such as optim)
        ...
        # note: keep --fp16 notification in v1.0
        if self.cuda and torch.cuda.get_device_capability(0)[0] >= 7:
            logger.info("NOTE: your device may support faster training with --fp16 or --amp")
        self._optimizer = loss.build_optimizer(self.cfg.optimizer, params)
        ...
        # note: this fairseq only implementation
        ...
        self._lr_scheduler = loss.build_lr_scheduler(
    ...
    # note: not sure why we have OSS - remove this and no_save_optimizer_state
    ...
    def state_dict(self):
        """
        Note: State dict defines what the models param file will contain.
        ...
        state_dict = {
            ...
            "criterion": (
                ...
                if helpers.has_parameters(self.criterion)
                else None
            ),
        ...
        # note: saving optim state in ckpt
    ...
    def save_checkpoint(self, filename, extra_state):
        ...
        state_dict = helpers.move_to_cpu(self.state_dict())
        state_dict["extra_state"].update(extra_state)

        # note: assumed this for distributed, rank?
    ...
    def load_checkpoint(
        ...
        if bexists:
            # note: using only ddp based dist training - to avoid param/ckpt broadcasting only gradients b/n processes/gpu's
            # don't load on all ranks ckpt/params as we are not using fsdp
            # load_on_all_ranks = (
            ...
            # note: remove all ckpt loading option for dist. training (fsdp part)
            # if load_on_all_ranks or self.data_parallel_rank == 0:
            #     state = checkpoint.load_checkpoint_to_cpu(
            ...
            # Note: working my way to have only ddp
            last_optim_state = None
            state = None # dict for named model state

            # note: load on 0 and broadcast on other gpu's
            if is_distributed: #and not load_on_all_ranks:
                # todo: import important distributed scripts in utils/distributed.py
                ...
                if self.data_parallel_rank > 0: # not sure rank ref device or gpu id.
                    last_optim_state = state.get("last_optimizer_state", None)
            ...
            try:
                ...
                del state["model"]
                if helpers.has_parameters(self.get_criterion()):
        ...
        if last_optim_state is not None and not reset_optimizer:
            ...
            ), f"Criterion does not match; please reset the optimizer (--reset-optimizer). " \
    ...
    def get_train_iterator(
        ...
        :param load_dataset:
        :param data_selector:
        :param shard_batch_itr: #FIXME: clarity b/n data/model parallel in multi-gpu vs. distributed training and sharded data?
        :param disable_iterator_cache:
        :return:
        """
        if load_dataset:
        ...
        batch_iterator = self.task.get_batch_iterator(
            ...
            max_positions=helpers.resolve_max_positions(
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        from src.utils.helpers import NanDetector
        overflow = False
        try:
            ...
            # if not self.tpu: # fixme: re-arrange block
            #     if (
            #         not self.cfg.optimization.use_bmuf
            ...
            # out where it fails
            self.zero_grad()
            with NanDetector(self.get_model()): # fixme:
    ...
    # note: i don't keep the code i don't understand
    ...
    def _set_seed(self):
        ...
        # note: move seeding to helpers or initilization.py
        from src.utils.helpers import seeding
        seeding(seed)
    ...
    # note: remove extra featuers about logging
...
# note: find out why we need these func - important for basics?
...
# func's for checks on shared parameters
